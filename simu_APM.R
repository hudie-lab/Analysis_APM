...
## due to additive, parent-of-origin and maternal genetic effects without fixed effects.

## The functions follows the work of Laurin et al. (2018) (https://github.com/amatrhr/g-remladp)

Cc_generate <- function(dir,times,hsqA=0.4,hsqP=0.05,hsqM=0.1,K=0.1,P=0.5,nsnps=10000,nobs){
  ...
  while(tag <= nobs ){
    ...
    ### threshold for the liability
    thres <- qnorm(1-K)  
    
    ### generate the continuous liability
    liability <- Aeps %*% matrix(betaA, ncol = 1) + Peps %*% matrix(betaP, ncol = 1) + Meps %*% matrix(betaM, ncol = 1) + rnorm(1, mean = 0, sd = sqrt( 1 - (hsqA + hsqP + hsqM)))
    pheno <- as.integer(I (liability > thres ))
...
estima_CC <- function(dir1,times,nobs,K,P=0.5){
  ...
  system(paste0(cmd_gcta,
                paste0("./gcta64 --reml --reml-no-constrain  --mgrm-bin ",dir,"simgrmlist.", times,
                       ".txt --pheno ",dir ,simname,"_",times,
                       ".pheno  --prevalence ", K , " --out ",dir,simname,"_APM_", times)), 
         intern = TRUE)
  ...
  sink(paste0(dir,simname,"_PCGC_", times,".txt"))
  print(summary(fit))
  sink()
